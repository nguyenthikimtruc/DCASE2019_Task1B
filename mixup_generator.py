...
class MixupGenerator_Snapshot():
    def __init__(self, X_train, y_train, n_members, batch_size=32, alpha=0.2, shuffle=True, datagen=None):
        self.n_cycles = n_members
        self.X_train = X_train
        self.y_train = y_train
        self.batch_size = batch_size
        self.alpha = alpha
...
#                X_list = None
                batch_ids = indexes[i * self.batch_size * 2:(i + 1) * self.batch_size * 2]
                X, y = self.__data_generation(batch_ids)
#                for rp in range(1, self.n_cycles,1):
#                    X_list = list(X_list.append(X))
#                    print('loop {}'.format(rp))
#                print(len(X_list))
...
class MixupGenerator_Snapshot_():
...
#                print(len(X_list))
#                print(X[0].shape)
#                X = [X[i] for i in range(self.cycles)]
#                yield X_list, y
#                yield [X[0], X[1], X[2], X[3], X[4]], y
#                yield X, y
#                yield X,X,X,X,X, y
                yield ([np.array(X) for rp in range(self.n_cycles)],y)
    ...
    def __data_generation(self, batch_ids):
        ...
        X11 = self.X_train[batch_ids[:self.batch_size]]
        X12 = self.X_train[batch_ids[self.batch_size:]]
        X1 = X11 * X_l + X12 * (1 - X_l)
