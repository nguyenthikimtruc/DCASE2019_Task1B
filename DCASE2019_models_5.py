...
import keras
from keras.layers import Conv2D, BatchNormalization, Activation, GlobalAveragePooling2D, MaxPooling2D
from keras.layers import AveragePooling2D, Input, concatenate, Lambda, Dense, Convolution2D
...
#network definition
def model_best2019_base1(x1, BNaxis, n_labels, wd, name='best2019_base1_fcl'):
    ...
    x1 = BatchNormalization(axis=BNaxis)(x1)
...
def model_best2019_base1_MoE(x1, BNaxis,n_layerexperts,n_labels, wd, name='best2019_base1_fcl'):
    # DOnt use Input layer for MoE, Should use outside
    # first layer has 16 convolution filters 
    x1_tensor = Input(x1.shape[1:], name='Input1')
    x1 = Convolution2D(16, kernel_size=3, strides=1, data_format='channels_last', 
    ...
    if n_layerexperts==0:
        ...
        x1 = DenseMoE(64, n_experts = n_layerexperts, expert_activation='relu', gating_activation='softmax')(x1)
    # Softmax
    output = Dense(n_labels, activation='softmax')(x1)
...
def model_best2019_base2(x1, BNaxis, n_labels, wd, name='best2019_base2'):
    # DOnt use Input layer for MoE, Should use outside
    # first layer has 16 convolution filters 
    x1_tensor = Input(x1.shape[1:], name='Input1')
    x1 = Convolution2D(16, kernel_size=3, strides=1, data_format='channels_last', 
...
def model_best2019_base2_MoE(x1, BNaxis,n_layerexperts, n_labels, wd, name='best2019_base2'):
    # DOnt use Input layer for MoE, Should use outside
    # first layer has 16 convolution filters 
    x1_tensor = Input(x1.shape[1:], name='Input1')
    x1 = Convolution2D(16, kernel_size=3, strides=1, data_format='channels_last', 
    ...
    if n_layerexperts==0:
        ...
        x1 = DenseMoE(128, n_experts = n_layerexperts, expert_activation='relu', gating_activation='softmax')(x1)
...
def create_model_db_3cnn_nexp(x1, BNaxis, n_layerexperts, n_labels, name='db_3cnn'):
    # DOnt use Input layer for MoE, Should use outside
    # first layer has 32 convolution filters 
    x1_tensor = Input(x1.shape[1:], name='Input1')
    x1 = BatchNormalization(axis=BNaxis)(x1_tensor)
    x1 = Convolution2D(32, kernel_size=3, kernel_initializer='glorot_uniform')(x1_tensor)
    x1 = Activation('relu')(x1)
    x1 = BatchNormalization(axis=BNaxis)(x1)
    x1 = Convolution2D(32, kernel_size=3)(x1)
    x1 = Activation('relu')(x1)
    x1 = MaxPooling2D(pool_size=(3,3))(x1)
    
    # next layer has 128 convolution filters
    x1 = BatchNormalization(axis=BNaxis)(x1)
    x1 = Convolution2D(128, kernel_size=3, kernel_initializer='glorot_uniform')(x1)
    x1 = Activation('relu')(x1)
    x1 = BatchNormalization(axis=BNaxis)(x1)
    x1 = Convolution2D(128, kernel_size=3)(x1)
    x1 = Activation('relu')(x1)
    x1 = MaxPooling2D(pool_size=(3,3))(x1)
    
    # next layer has 256 convolution filters
    x1 = BatchNormalization(axis=BNaxis)(x1)
    x1 = Convolution2D(256, kernel_size=3, kernel_initializer='glorot_uniform')(x1)
    x1 = Activation('relu')(x1)
    x1 = BatchNormalization(axis=BNaxis)(x1)
    x1 = Convolution2D(256, kernel_size=3)(x1)
    x1 = Activation('relu')(x1)
    x1 = GlobalAveragePooling2D(data_format='channels_last')(x1)
    # interpretation model
    if n_layerexperts==0:
